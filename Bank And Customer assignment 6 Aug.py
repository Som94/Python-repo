...
class Bank:
    ...
    def deposit(self,amount):
        if amount not in range(100,1000000):
            with open('tran_failure.txt','a+') as f:
                f.write('\n'+str(self.acno))
                f.write(', Deposit, ')
                f.write(str(amount))
                if amount<100:
                    f.write(', amount cannot be less than 100,')
                else:
                    f.write(', amount cannot be greater than 1000000,') 
                f.write(str(datetime.now()))
            raise AmountDipositException('Deposite Amount Should be in range 100 to 1000000')
        ...
        if amount in range(100,500000):
            ...
            if Customer_ac_Bal < 1000:
                ...
                with open('tran_failure.txt','a+') as f:
                    ...
                    f.write(', Withdraw,')
                    f.write(str(amount ))
                    f.write(', min balance is not met, ')
                    f.write(str(datetime.now()))
            ...
            with open('tran_failure.txt','a+') as f:
                    ...
                    if amount<100:
                        ...
                        f.write(', amount cannot be greater than 500000, ')
                    f.write(str(datetime.now()))
